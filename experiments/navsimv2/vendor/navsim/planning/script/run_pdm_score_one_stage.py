...
def _maybe_visualize_sample(
    ...
    visualize = getattr(agent, "maybe_visualize_pdm_score_sample", None)
    should_visualize = getattr(agent, "should_visualize_pdm_score_sample", None)
    if (
        not getattr(agent, "eval_visualization_enabled", False)
        or not callable(visualize)
        or (callable(should_visualize) and not should_visualize())
    ):
        return
    ...
    try:
        ...
        visualize(
            ...
            sample_idx=int(getattr(agent, "_eval_visualization_saved_samples", 0)),
            metrics=metrics,
            stage_name="one_stage",
            frame_type=str(metric_cache.scene_type),
        )
    except Exception:
        logger.exception("Failed to render official NAVSIM BEV for token %s", token)
...
def _run_evaluation(cfg: DictConfig) -> tuple[float, Path]:
    """
    ...
    average_score = float(
        pdm_score_df.loc[pdm_score_df["token"] == "average_all_frames", "score"].iloc[0]
    )
    ...
    return average_score, result_path


def run_human_gt_evaluation(cfg: DictConfig) -> tuple[float, Path]:
    """Run the full official one-stage scorer with the human future trajectory."""
    gt_cfg = deepcopy(cfg)
    human_agent_config = Path(__file__).resolve().parent / "config/common/agent/human_agent.yaml"
    with open_dict(gt_cfg):
        gt_cfg.agent = OmegaConf.load(human_agent_config)
        if cfg.agent.get("num_future_frames") is not None and cfg.agent.get("fps") is not None:
            num_future_frames = int(cfg.agent.num_future_frames)
            fps = float(cfg.agent.fps)
            gt_cfg.agent.trajectory_sampling.time_horizon = num_future_frames / fps
            gt_cfg.agent.trajectory_sampling.interval_length = 1.0 / fps
        gt_cfg.experiment_name = f"{cfg.experiment_name}_gt"
        gt_cfg.output_dir = str(Path(cfg.output_dir) / "gt")

    Path(gt_cfg.output_dir).mkdir(parents=True, exist_ok=True)
    logger.info("Starting full NAVSIM v2 navtest GT evaluation: output=%s", gt_cfg.output_dir)
    return _run_evaluation(gt_cfg)


@hydra.main(config_path=CONFIG_PATH, config_name=CONFIG_NAME, version_base=None)
def main(cfg: DictConfig) -> None:
    model_score, model_path = _run_evaluation(cfg)
    if str(cfg.agent.get("_target_", "")) == "navsim.agents.human_agent.HumanAgent":
        return
    if not EVALUATE_HUMAN_GT:
        logger.info("Skipping NAVSIM v2 navtest GT evaluation.")
        return
    gt_score, gt_path = run_human_gt_evaluation(cfg)
    logger.info(

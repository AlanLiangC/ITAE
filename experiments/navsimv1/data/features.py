...
class NavsimV1FeatureBuilder(AbstractFeatureBuilder):
    """Emits front-camera history and ego-status tensors."""

    def __init__(self, config: NavsimV1FeatureConfig):
        self._config = config

    def get_unique_name(self) -> str:
        return "navsim_v1_feature"
    ...
    def _front_frames(self, agent_input: AgentInput) -> List[torch.Tensor]:
        ...
        if not frames:
            raise RuntimeError("NAVSIM v1 adapter did not receive any front-camera history frames.")
        return frames
    ...
    def _front_projection_metadata(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        if camera.image is None:
            raise RuntimeError("NAVSIM v1 adapter expected a loaded cam_f0 image.")
        if (
    ...
    def _ego_status_tensor(agent_input: AgentInput) -> torch.Tensor:
        ...
        if not ego_features:
            raise RuntimeError("NAVSIM v1 adapter did not receive any ego-status history.")

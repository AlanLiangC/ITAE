...
class FutureTransitionPredictor(nn.Module):
    ...
    def __init__(
        self, latent_dim: int, model_dim: int, num_future_frames: int, num_heads: int
    ) -> None:
        super().__init__()
        ...
        self.output = nn.Sequential(nn.LayerNorm(model_dim), nn.Linear(model_dim, model_dim))
...
class VisionActionTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.resampler = SpatialResampler(
            pe_feature_dim, model_dim, resampled_tokens_per_frame, num_heads, dropout
        )
        ...
        self.transition_predictor = FutureTransitionPredictor(
            latent_dim, model_dim, num_visual_frames - 1, num_heads
        )
    ...
    def forward(
        ...
        current_global = frame_tokens[:, 0].mean(dim=1, keepdim=True)
        target_transition = frame_tokens[:, 1:].mean(dim=2) - current_global
